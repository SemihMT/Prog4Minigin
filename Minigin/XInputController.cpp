...
public:
	...
	void Update()
	{
		//copy currentstate to previous state
		CopyMemory(&m_PreviousState, &m_CurrentState, sizeof(XINPUT_STATE));
		//fill current state with zeros
		ZeroMemory(&m_CurrentState, sizeof(XINPUT_STATE));

		//Get the current input state of this controller:
		DWORD dwResult = XInputGetState(m_ControllerIdx, &m_CurrentState);
...
XInputController::XInputController(int controllerIdx) : m_pImpl(new XInputControllerImpl(controllerIdx))
{
}
