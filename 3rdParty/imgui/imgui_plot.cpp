...
PlotStatus Plot(const char* label, const PlotConfig& conf) {
    ...
    if (conf.values.count > 0) {
        ...
        float x_max = static_cast<float>(conf.values.offset) + static_cast<float>(conf.values.count) - 1;
        if (conf.values.xs) {
        ...
        if (conf.grid_x.show) {
            ...
            case PlotConfig::Scale::Linear: {
                float cnt = static_cast<float>(conf.values.count) / static_cast<float>((static_cast<float>(conf.grid_x.size) / static_cast<float>(conf.grid_x.subticks)));
                float inc = 1.f / cnt;
                for (int i = 0; i <= cnt; ++i) {
            ...
            case PlotConfig::Scale::Log10: {
                ...
                while (start < x_max) {
                    for (int i = 1; i < 10; ++i) {
                        ...
                        if (x < x_min) continue;
                        if (x > x_max) break;
                        float t = static_cast<float>(log10(static_cast<double>(x / x_min))) / static_cast<float>(log10(static_cast<double>(x_max / x_min)));
